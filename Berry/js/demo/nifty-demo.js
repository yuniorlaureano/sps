...
$(document).ready(function () {
	...
	var themeBtn = $('.demo-theme'),
      changeTheme = function (themeName, type, path) {
          if (path == undefined) {
              path = $('div[data-path]').data('path');
          }
          var themeCSS = $('#theme'),
                 filename = path + '/type-' + type + '/' + themeName + '.min.css';
	...
	$('#demo-theme').on('click', '.demo-theme', function (e) {
	    ...
	    changeTheme(el.attr('data-theme'), el.attr('data-type'), $('#demo-theme').attr("data-path"));
	    themeBtn.removeClass('disabled');
	    el.addClass('disabled');
	    $('.brand-icon').css('backgroundColor', $('.navbar-content').css('backgroundColor'));
	    return false;
	});
	...
	var activeLink = function (name) {
	    ...
	    var activeLi = $("#mainnav-menu").find("li[data-name='" + name + "']");

	    activeLi.parents("ul").collapse('show');

	    if (!(activeLi.children("ul")).length) {

	        activeLi.addClass("active-link");
	    }

	    setCookie("settings-nav-active", name);

	    return activeLi.find("a").attr("href");
	}
	  
	var validateActiveLocation = function (actualLocation) {
	...
	if ($.cookie('settings-nav-active')) {
	    var href = activeLink($.cookie('settings-nav-active'));
	   // validateActiveLocation(href)
	} 
